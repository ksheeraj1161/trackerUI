...
def load_csv(path):
    with open(path, newline="", encoding="utf-8-sig") as f:
        ...
        for h in raw_headers:
            ...
            n = _norm(h)
            if n and n not in norm2orig: norm2orig[n] = h
        rows = []
        for r in rdr:
            row = {}
            for k, v in r.items():
...
def make_table(headers, row):
    def esc(x): return html.escape(str(x) if x is not None else "")
    if not row: return ""
    parts = ["<table class='kv'><tbody>"]
    for h in headers:
        val = row.get(h, "")
        if isinstance(val, str) and not val.strip():
            continue
        parts.append(f"<tr><th>{esc(h)}</th><td>{render_value(h,val)}</td></tr>")
    parts.append("</tbody></table>")
    return "".join(parts)

def page_html(headers, row, id_key, error=None, q=""):
    def esc(x): return html.escape(str(x) if x is not None else "")
    ...
    details_tbl = make_table(left_headers, row) if row else ""
    docs_tbl    = make_table(right_headers, row) if row else ""
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .acc-btn.open .chev {{ transform: rotate(45deg); }} /* downwards when open */
...
<script>
  ...
  // Optional: auto-open left panel after a successful search
  (function(){
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        q = (parse_qs(p.query).get("id") or [""])[0]

        try:
            headers, norm2orig, rows = load_csv(CSV_PATH)
            id_key = detect_id_key(norm2orig, headers)
            row = find_row(rows, id_key, q) if (id_key and q) else None
            page = page_html(headers, row, id_key, None, q)
            self._send(200, page)
        except FileNotFoundError:
            page = page_html([], None, None, f"CSV not found at '{CSV_PATH}'.", q)
            self._send(500, page)
        except Exception as e:
            page = page_html([], None, None, str(e), q)
            self._send(500, page)
